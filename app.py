...
def run_download(job_id, url, format_choice, format_id):
    ...
    try:
        ...
        if result.returncode != 0:
            job["status"] = "error"
            # Capture last 2 lines of stderr for more context
            error_msg = result.stderr.strip().split("\n")
            job["error"] = " | ".join(error_msg[-2:]) if len(error_msg) >= 2 else (error_msg[-1] if error_msg else "Unknown error")
            return
...
if __name__ == "__main__":
    # Ép kiểu int cho port và listen trên 0.0.0.0
